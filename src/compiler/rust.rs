...
fn strip_basedirs_in_arg_impl<'a>(arg: &'a [u8], basedirs: &[Vec<u8>]) -> Cow<'a, [u8]> {
    ...
    match find_and_strip_basedirs(&normalized, basedirs) {
        // No match on the normalized view: fall back to the original arg,
        // mirroring `strip_basedir_prefix`.
        Cow::Borrowed(_) => Cow::Borrowed(arg),
        // Matched: the returned bytes point into the normalized buffer, so
        // we must return an owned copy.
...
";
    ...
    fn test_strip_basedirs_in_arg_prefix() {
        // Plain source-file arg: basedir at position 0.
        let out =
    ...
    fn test_strip_basedirs_in_arg_multiple_in_one() {
        // Two occurrences in a single arg: both strip.
        let out =
    ...
    fn test_strip_basedirs_in_arg_no_match_inside() {
        // Not at a boundary (preceded by letter/digit/slash) -> not stripped.
        // Mirrors strip_basedirs' boundary semantics for C preprocessor output.
        let out =
